...
def product(request, id):
    ...
    try:
        in_cart = CartItem.objects.get(user = request.user, product = product_object).quantity
    except:
        in_cart = 0
    ...
    if request.POST.get('counter'):
        if product_object.quantity >= int(request.POST.get('counter')) + in_cart:
            try:
                cart_object = CartItem.objects.get(user = request.user, product = product_object)
                cart_object.quantity += int(request.POST.get('counter'))
                cart_object.save()
                
            except:
                cart_object = CartItem(product=product_object, user=request.user, quantity=request.POST.get('counter'))
                cart_object.save()
            in_cart += int(request.POST.get('counter'))
            messages.success(request, (f"Dodano do koszyka {request.POST.get('counter')} {product_object.title}"))
        else:
...
def cart(request):
    ...
    context = {
        'objects': objects,
        'price_for_everything': price_for_everything,
    }
